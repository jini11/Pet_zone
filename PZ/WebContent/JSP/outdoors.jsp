	...
	<div class="container">
		<div class="row" style="margin:50px auto auto 180px;">
			...
			<%
				if(pageNumber != 1) {
			%>
				<a href="outdoors.jsp?pageNumber=<%=pageNumber - 1 %>&area1=<%=area1%>&area2=<%=area2%>" class="btn btn-success btn-arraw-left">이전</a>
			<%
				} if(pzDAO.nextPage(pageNumber + 1)) {
			%>
				<a href="outdoors.jsp?pageNumber=<%=pageNumber + 1 %>&area1=<%=area1%>&area2=<%=area2%>" class="btn btn-success btn-arraw-left" >다음</a>
			<%
				}
			%>
