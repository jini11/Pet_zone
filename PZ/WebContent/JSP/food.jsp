		...
		<div class="row" style="margin:50px auto auto 180px;">
			<table class="table table-striped table-bordered table-hover fz_table" style="width: 1600px; text-align:center; border-collapse: collapse; padding-bottom: 30px;">
				...
				<tbody>
					...
					ArrayList<Pz> list = pzDAO.getList(pageNumber,"food", str, area2);
			...
			<%
				if(pageNumber != 1) {
			%>
				<a href="food.jsp?pageNumber=<%=pageNumber - 1 %>&area1=<%=area1%>&area2=<%=area2%>" class="btn btn-success btn-arraw-left">이전</a>
			<%
				} if(pzDAO.nextPage(pageNumber + 1)) {
			%>
				<a href="food.jsp?pageNumber=<%=pageNumber + 1 %>&area1=<%=area1%>&area2=<%=area2%>" class="btn btn-success btn-arraw-left" >다음</a>
			<%
				}
			%>
