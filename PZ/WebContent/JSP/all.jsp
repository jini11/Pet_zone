...
<body>
	...
	<div>
		...
		<header class="header">
			...
			<nav class="nav">
				...
				<a href="login.jsp" class="button_login">LOGIN</a>
			</nav>
			<!-- 네비게이션 영역 끝 -->
		</header>
		<!-- 헤더영역 끝 -->
	...
	<div class="search">
		<div style="margin: 50px auto auto 180px;">
			<form name="select_area">
				<table class="area"
					style="width: 1100px; height: 80px; text-align: center; border-collapse: collapse;">
					<thead>
	...
	<div class="container">
		<div class="row" style="margin:50px auto auto 180px;">
			<table class="table table-striped table-bordered table-hover fz_table" style="width: 1600px; text-align:left; border-collapse: collapse; padding-bottom: 30px;">
				<thead>
					<tr>
				...
				<tbody>
					...
					ArrayList<Pz> list = pzDAO.getList(pageNumber,"all", str, area2);
			...
			<%
				if(pageNumber != 1) {
			%>
				<a href="all.jsp?pageNumber=<%=pageNumber - 1 %>&area1=<%=area1%>&area2=<%=area2%>" class="btn btn-success btn-arraw-left">이전</a>
			<%
				} if(pzDAO.nextPage(pageNumber + 1)) {
			%>
				<a href="all.jsp?pageNumber=<%=pageNumber + 1 %>&area1=<%=area1%>&area2=<%=area2%>" class="btn btn-success btn-arraw-left" >다음</a>
			<%
				}
			%>
